...
def main():
    ...
    dload.add_argument("-c", "--chunk-size", type=int, help="download chunk size in MB (default: 8)", default=8)
    dload.add_argument("-t", "--threads", type=int, help="number of parallel download threads (default: 4, use 1 to disable)", default=4)
...
def download(args):
    ...
    if use_parallel:
        print(f"Parallel Download : {args.threads} threads")
    ...
    log_to_file(f"Threads: {args.threads}")
    ...
    # Use parallel download if threads > 1 and not resuming
    if use_parallel and not args.resume:
        download_parallel(client, path, filename, size, out, args)
    else:
        if use_parallel and args.resume:
            print("Note: Parallel download not supported with resume, using single-threaded download")
            log_to_file("Parallel download disabled for resume")
        download_single(client, path, filename, size, dloffset, out, args)
...
def download_parallel(client, path, filename, size, out, args):
    ...
    for i in range(num_threads):
        ...
        # Last thread gets remaining bytes
        end = size if i == num_threads - 1 else (i + 1) * segment_size
        ranges.append((i, start, end))
    
    # Create output file with proper size using truncate
    with open(out, 'wb') as f:
        f.truncate(size)
    
    log_to_file(f"Starting parallel download with {num_threads} threads")
    
    # Progress tracking
    lock = threading.Lock()
    progress_tracker = {'downloaded': 0}
    log_interval = size // 10
    log_interval_update = size // 10  # Calculate once outside the loop
    chunk_size_bytes = args.chunk_size * 1024 * 1024
    
    def download_range(thread_id, start, end):
        """Download a specific range of the file."""
        try:
            # Create separate client for this thread to avoid conflicts
            thread_client = fusclient.FUSClient()
            initdownload(thread_client, filename)
            
            response = thread_client.downloadfile(path+filename, start)
            
            bytes_to_download = end - start
            bytes_downloaded = 0
            ...
            with open(out, 'r+b') as f:
                ...
                for chunk in response.iter_content(chunk_size=chunk_size_bytes):
                    if chunk:
                        ...
                        remaining = bytes_to_download - bytes_downloaded
                        chunk_to_write = chunk[:remaining]
                        
                        f.write(chunk_to_write)
                        bytes_downloaded += len(chunk_to_write)
                        
                        with lock:
                            progress_tracker['downloaded'] += len(chunk_to_write)
                            pbar.update(len(chunk_to_write))
                            
                            # Log progress
                            if progress_tracker['downloaded'] >= log_interval:
                                log_to_file(f"Download progress: {progress_tracker['downloaded'] / (1024**2):.2f} MB / {size / (1024**2):.2f} MB")
                                while progress_tracker['downloaded'] >= log_interval:
                                    log_interval += log_interval_update
                        
                        if bytes_downloaded >= bytes_to_download:
                            break
                            
        except Exception as e:
            log_to_file(f"Thread {thread_id} error: {e}")
            raise
    
    # Start download with progress bar
    with tqdm(total=size, unit="B", unit_scale=True) as pbar:
        threads = []
        for thread_id, start, end in ranges:
            t = threading.Thread(target=download_range, args=(thread_id, start, end))
            t.start()
            threads.append(t)
...
def download_single(client, path, filename, size, dloffset, out, args):
    """Download file using single connection (original method)."""
    fd = open(out, "ab" if dloffset > 0 else "wb")
    ...
    # Convert chunk size from MB to bytes (default 8MB for better performance)
    chunk_size_bytes = args.chunk_size * 1024 * 1024
