...
if [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "essi" ]]; then
    RC_FILE="init.$(GET_PROP "vendor" "ro.product.board").rc"
elif [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "mssi" ]]; then
    RC_FILE="hw/init.project.rc"
elif [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "qssi" ]]; then
    RC_FILE="hw/init.target.rc"
fi
if [ "$RC_FILE" ] && [ -f "$WORK_DIR/vendor/etc/init/$RC_FILE" ]; then
    if ! grep -q "persist.vendor.radio.port_index" "$WORK_DIR/vendor/etc/init/$RC_FILE"; then
        {
            echo ""
            echo "on property:persist.vendor.radio.port_index=\"\""
            echo "    setprop sys.usb.config adb"
        } >> "$WORK_DIR/vendor/etc/init/$RC_FILE"
    fi
    unset RC_FILE
fi
