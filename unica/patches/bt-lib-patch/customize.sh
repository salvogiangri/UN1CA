...
if [ ! -f "$WORK_DIR/system/system/lib64/libbluetooth_jni.so" ]; then
    ...
    echo "/system/system/lib64/libbluetooth_jni\.so u:object_r:system_lib_file:s0" >> "$WORK_DIR/configs/file_context-system"
    echo "system/system/lib64/libbluetooth_jni.so 0 0 644 capabilities=0x0" >> "$WORK_DIR/configs/fs_config-system"
fi
