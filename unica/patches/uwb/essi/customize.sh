...
if [ -f "$FW_DIR/${MODEL}_${REGION}/system/system/etc/libuwb-cal.conf" ]; then
    ...
    if [ ! -d "$FW_DIR/$SOURCE_FIRMWARE_PATH/system/system/app/UwbUci" ]; then
        cp -a --preserve=all "$SRC_DIR/unica/patches/uwb/essi/system/"* "$WORK_DIR/system/system"
        ...
        if $TARGET_HAS_SYSTEM_EXT; then
            cp -a --preserve=all "$SRC_DIR/unica/patches/uwb/essi/system_ext/"* "$WORK_DIR/system_ext"
            ...
            cp -a --preserve=all "$SRC_DIR/unica/patches/uwb/essi/system_ext/"* "$WORK_DIR/system/system/system_ext"
