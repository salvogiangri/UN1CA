...
if [[ ! -f "$WORK_DIR/system/system/lib64/libnfc_${TARGET_LIB_NAME}_jni.so" ]]; then
    ...
    # Workaround of pre-U libs
    sed -i "s/\<CoverAttached\>/coverAttached/g" "$WORK_DIR/system/system/libnfc_${TARGET_LIB_NAME}_jni.so"
    sed -i "s/\<StartLedCover\>/startLedCover/g" "$WORK_DIR/system/system/libnfc_${TARGET_LIB_NAME}_jni.so"
    sed -i "s/\<StopLedCover\>/stopLedCover/g" "$WORK_DIR/system/system/libnfc_${TARGET_LIB_NAME}_jni.so"
    sed -i "s/\<TransceiveLedCover\>/transceiveLedCover/g" "$WORK_DIR/system/system/libnfc_${TARGET_LIB_NAME}_jni.so"
